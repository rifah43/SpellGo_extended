class GameSucceed extends Phaser.Scene {
  ...
  create() {
    ...
    this.add
      .text(config.width / 2, 120, "TASK COMPLETED!", {
    ...
    new MessageBox({
      ...
      fontFamily: "PressStart",
      fontSize: 15,
    }).startTyping(this.todo);

    this.add
      .text(
        config.width / 2,
        config.height / 2 - 64,
        this.coinAmount + " coins collected!",
        {
          fontFamily: "PressStart",
          fontSize: 15,
          fill: "#ffffff",
    ...
    this.rotatingCoin = this.add
      .sprite(config.width / 2, config.height / 2, "coin")
      .setOrigin(0.5);
    this.rotatingCoin.play("coin");

    this.rotatingCoin1 = this.add
      .sprite(config.width / 2 - 44, config.height / 2, "coin")
      .setOrigin(0.5);
