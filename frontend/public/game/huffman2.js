class Huffman2 {
  ...
  startPhaseTwo() {
    ...
    this.ctx.add
      .rectangle(
        config.width / 2,
        (this.ctx.frequency.length * 45) / 2 + 10,
        200,
        this.ctx.frequency.length * 45,
        0xffffff
      )
      .setOrigin(0.5);
    ...
    for (let i = 0; i < this.ctx.frequency.length; i++) {
      ...
      const encode = this.ctx.add
        .text(config.width / 2 - 50, 20 + 40 * i, value + ": ", {
    ...
    this.one.getByName("btn").on("pointerdown", () => {
      if (this.node.right) {
        ...
        if (
          this.node.value &&
          this.node.value == this.values[this.currentIndex]
        ) {
      ...
      try {
          ...
          console.error('Error:', error);
                ...
                this.ctx.scene.start("gameSucceed", {
                  ...
                  todo: [
                    ...
                    {
                      text: "Enter into the post office",
          ...
          setTimeout(() => {
            ...
            new MessageBox({ ctx: this.ctx }).startTyping(
              window.hints.huffman.hint3
            );
            this.encoding[this.currentIndex].setText(
