class Dijkstra extends Phaser.Scene {
  ...
  createMap() {
    ...
    this.add
      .sprite(this.widthRatio * 26 * 16, this.heightRatio * 11 * 16, "fountain")
      .play("fountain");
  }
  createWizards() {
    for (let i in this.bridgeMap) {
      ...
      let wizard;
      if (shouldMirror == 0) {
        wizard = this.add.sprite(
          this.widthRatio * wizX * 16,
          this.heightRatio * wizY * 16,
          "badWizardAttack"
        );
      } else {
        wizard = this.add.sprite(
          this.widthRatio * (wizX - 2) * 16,
          this.heightRatio * wizY * 16,
          "badWizardAttack"
        );
        wizard.flipX = true;
      ...
      const text = this.add.text(
        wizard.x - 35,
        wizard.y - 40,
        " Wizard Point: " + this.bridgeMap[i].cost,
        {
          font: "9px CustomFont",
          fill: "#000000",
          align: "center",
  ...
  getBridgeInfo(sourceVillage, destinationVillage) {
    let name =
      Math.min(sourceVillage, destinationVillage) +
      "," +
      Math.max(sourceVillage, destinationVillage);
  ...
  async updateBtnAction(destinationVillage) {
    ...
    if (key.destinationCost > key.sourceCost + key.bridgeCost) {
      ...
      this.map[destinationVillage].banner
        .getChildren()[1]
        .setText(
          this.map[destinationVillage].name +
            " " +
            this.map[destinationVillage].cost
        );

      const upBridge = this.bridgeMap[key.name].bridge.texture.key.replace(
        "selected",
        "selectedVisited"
      );
      ...
      if (prevParent != -1) {
        ...
        const removeBridge = this.bridgeMap[
          prevBridge.name
        ].bridge.texture.key.replace("Visited", "");
      ...
      await new MessageBox({ ctx: this }).startTyping([
        {
          text:
            "You will have to spend much wizard point then before if you go through city " +
            this.map[this.sourceVillage].name +
            " to city " +
            this.map[destinationVillage].name +
            "!",
  ...
  async ignoreBtnAction(destinationVillage) {
    ...
    if (key.destinationCost > key.sourceCost + key.bridgeCost) {
      await new MessageBox({ ctx: this }).startTyping([
        {
          text:
            "You will have to spend less wizard point then before if you go through the city " +
            this.map[this.sourceVillage].name +
            " to the city " +
            this.map[destinationVillage].name +
            "!",
  ...
  operationDone(destinationVillage, key) {
    ...
    if (
      this.map[this.sourceVillage].adjTracked ==
      this.map[this.sourceVillage].adjacent.length
    ) {
      ...
      if (srcParent != -1) {
        const confirmedBridge = this.getBridgeInfo(
          srcParent,
          this.sourceVillage
        );
        ...
        this.bridgeMap[confirmedBridge.name].bar
          .getChildren()[0]
          .play("wizardPointBar");
  ...
  defineAdjacentResponsibility(villageNo, x, y) {
    ...
    updateBtn
      .getByName("btn")
      .on("pointerdown", this.updateBtnAction.bind(this, villageNo));
    ignoreBtn
      .getByName("btn")
      .on("pointerdown", this.ignoreBtnAction.bind(this, villageNo));
  ...
  async defineResposibility(villageNo) {
    if (!this.adjacentChosen) {
      ...
      new MessageBox({ ctx: this }).startTyping(
        window.guidance.dijkstra.minAdj
      );
    }
  ...
  setDefault() {
    ...
    for (let i = 0; i < this.map[this.sourceVillage].adjacent.length; i++) {
      ...
      const bridgeNo =
        Math.min(this.sourceVillage, j) + "," + Math.max(this.sourceVillage, j);
      const defaultBridge = this.bridgeMap[bridgeNo].bridge.texture.key.replace(
        "selected",
        ""
      );
  ...
  createBanners() {
    for (let i = 0; i < this.map.length; i++) {
      ...
      let banner = this.add.image(
        this.widthRatio * bannerX * 16,
        this.heightRatio * bannerY * 16,
        "unselectedVillage"
      );
      ...
      this.defineAdjacentResponsibility(
        i,
        banner.x - 30,
        banner.y + banner.height * 0.4
      );
      ...
      const text = this.add.text(
        ...
        this.map[i].name +
          " " +
          (this.map[i].cost == Number.MAX_VALUE ? "inf" : this.map[i].cost),
        {
  ...
  createBridges() {
    for (let bridgeKey in this.bridgeMap) {
      ...
      let bridge = this.add.image(
        this.widthRatio * bridgeX * 16,
        this.heightRatio * 16 * bridgeY,
        state == "vertical" ? "VerticalBridge" : "HorizontalBridge"
      );
