class Huffman1 extends Phaser.Scene {
  ...
  create() {
    ...
    this.rewind = new Rewind({
      ctx: this,
      callback: () => {
        this.scene.start("game_over", { key: "huffman" });
      },
    });
    ...
    for (let i = 0; i < this.frequency.length; i++) {
      ...
      const freq = this.add
        .text(0, 0, key + ": " + this.frequency[i][key], {
          color: "#000000",
        })
        .setOrigin(0.5);

      const container = this.add.container(
        this.offset * i + this.offset / 2,
        this.minY + 15
      );
      container.add(node);
      container.add(freq);
        ...
        if (!this.pickedLeft) {
          ...
          this.tweens.add({
            ...
            onComplete: () => {
              ...
              const leftNode = this.tree.slice(
                this.leftInd,
                this.leftInd + 1
              )[0];
              const rightNode = this.tree.slice(
                this.rightInd,
                this.rightInd + 1
              )[0];
              const height = Math.max(leftNode.height, rightNode.height) + 1;

              const ind1 = Math.min(this.leftInd, this.rightInd),
                ind2 = Math.max(this.leftInd, this.rightInd);
                  ...
                  onComplete: () => {
                    if (j == this.tree.length - 1) {
                      ...
                      if (this.btns.length == 1) {
                        this.btns.pop().alpha = 0;
                        this.phase2 = new Huffman2({
                          ctx: this,
                        }, this.levelName).startPhaseTwo();
                      }
                    }
                  },
                });
  ...
  createBranch() {
    const node = this.add.rectangle(0, 0, 80, 30, 0xffffff).setOrigin(0.5);
    const rightNode = this.add
      .rectangle(this.disX, this.disY, 80, 30, 0xffffff)
      .setOrigin(0.5);
    const leftNode = this.add
      .rectangle(-this.disX, this.disY, 80, 30, 0xffffff)
      .setOrigin(0.5);
    ...
    const line = new Phaser.Geom.Line(
      ...
      node.height / 2,
      leftNode.x,
      leftNode.y - leftNode.height / 2
    );
    const line2 = new Phaser.Geom.Line(
      node.x,
      node.height / 2,
      rightNode.x,
      rightNode.y - rightNode.height / 2
    );
