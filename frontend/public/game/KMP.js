class KMP extends Phaser.Scene{
    constructor(){
        ...
        this.pt=["Yellow", "Yellow","Orange","Yellow","Yellow", "Yellow","Orange"];
    ...
    preload(){
        ...
        const graphics=this.add.rectangle(this.offset2-55,config.height/2-240,105*this.pt.length,150,0x008483).setOrigin(0);
        ...
        for(let i=0;i<this.pt.length;i++){
            ...
            const l= this.add.text(x,config.height/2-120, i, {
            ...
            this.anims.create({
                ...
                frames: this.anims.generateFrameNumbers("slime"+this.pt[i],{frames:frameSequence}),
                frameRate: 5,
                repeat: -1
            });
    ...
    create(){
        ...
        this.countdown=new CountdownController({ctx:this, duration:this.steps*7000});
        this.countdown.start();
        this.lps=this.add.group();

        this.preText= this.createText(0,config.height/2-60,"Longest Prefix",'#ffffff',10);
        this.preText.visible=false;

        this.suffText= this.createText(0,config.height/2-60,"Also Suffix",'#015453',10);
        this.suffText.visible=false;

        this.prefix=this.add.image(this.offset-50,config.height/2-50,"redLid").setOrigin(0);
        this.suffix=this.add.image(0,config.height/2-50,"blueLid").setOrigin(0);
        ...
        this.lpsBar.add(graphics);
        for(let i=0;i<this.pt.length;i++){
            const l= this.createText(this.offset+i*width,y+40,"?",'#ffffff',40);
            this.lps.add(l);
        ...
        graphics.fillRoundedRect(this.offset-50,config.height/2+35,100*this.pt.length,8,5);

        this.lengthBar=this.getBar(this.offset-50, config.height/2+35,"Potential Match!",0xFF1A4B);
        this.lengthBar.visible=false;
        this.indexBar=this.getBar(this.offset-50, config.height/2+35,"To be Matched",0x015453);
    ...
    getBar(x,y,name,color=0xffffff){
        ...
        const l=this.createText(50,30,name,this.decimalToHex(color),20,"CustomFont");
    ...
    update(){
        ...
        if( !this.backwardDone && this.i>0 && this.len<this.pt.length && this.pt[this.len]!=this.pt[this.i]){
            this.backwardDone=true;
            (new MessageBox({ctx:this,fontSize:15})).startTyping(window.guidance.kmp.guide3);
        }

        if( !this.updateDone && this.i>0 && this.pt[this.len]==this.pt[this.i]){
