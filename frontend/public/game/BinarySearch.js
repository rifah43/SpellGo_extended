class BinarySearch extends Phaser.Scene {
    ...
    createPapers() {
        this.papers = [];
        const potionNames = ['Elixirium', 'Magikor', 'Arcanite', 'Enchantus', 'Charmaze', 'Mysticon', 'Potentia', 'Spellbloom', 'Wizdor', 'Hexalite', 'Sorcera', 'Alchimix'];
        ...
        for (let i = 0; i < this.totalPaper; i++) {
            const page = this.add.image(0, 0/*offset+i*50,config.height/2*/, "paper");
            page.setOrigin(0, 0.5);
            page.setScale(0.5);

            const pageHeight = page.displayHeight * 0.5;
            const paperWidth = page.displayWidth;
            const pageNo = this.add.text(paperWidth - 30, pageHeight - 40, i + 1, {
                font: '14px WizardFont',
                fill: '#b81004',
                align: 'center'
            }).setName("page-number");

            const potionName = this.add.text(paperWidth / 2, -90, potions[i], {
    ...
    create() {
        ...
        this.left.on("pointerdown", () => {
            ...
            if (this.papers[mid].getByName("potion-name").text < this.searchObj) {
                ...
                (new MessageBox({ ctx: this })).startTyping(window.hints.binarySearch.pickRight)
        ...
        this.right.on("pointerdown", () => {
            ...
            if (this.papers[mid].getByName("potion-name").text >= this.searchObj) {
                this.rewind.reduceLive();
                (new MessageBox({ ctx: this })).startTyping(window.hints.binarySearch.pickLeft)
                this.halfChosen = false;
                return;
            }
        ...
        this.add.text(config.width / 2, 30, `First Page of "${this.searchObj}" Potion`, {
    ...
    async update() {
        ...
        if (!this.midChosen) {
            ...
            this.midChosen = true;
            const mid = this.getMid();
            if (!this.leftChosen && this.papers[mid].getByName("potion-name").text >= this.searchObj) {
                this.leftChosen = true;
                (new MessageBox({ ctx: this })).startTyping(window.guidance.binarySearch.pickLeft);
            }
            else if (!this.rightChosen) {
                this.rightChosen = true;
                (new MessageBox({ ctx: this })).startTyping(window.guidance.binarySearch.pickRight);
            }
