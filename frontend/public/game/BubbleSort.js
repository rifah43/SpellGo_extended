class BubbleSort extends Phaser.Scene {
  ...
  init(data) {
    this.stairColors = [
      0xed7f79, 0xed6861, 0xeb5e57, 0xeb534b, 0xed463e, 0xfc2b21, 0xed241a,
    ];
  ...
  createStair() {
    ...
    for (var i = 0; i < this.numberOfStairs; i++) {
      const stair = this.add.container(
        0,
        config.height * 0.7 - i * stairOffset
      );
  ...
  createLight() {
    ...
    for (var i = 0; i < this.numberOfLights; i++) {
      const l = this.add.image(
        this.stairWidth + this.stairs[i].x,
        0,
        "lightOff"
      );
      l.setOrigin(0, 0);
  ...
  createCountdownTimer() {
    let totalTime =
      ((this.numberOfLights * (this.numberOfLights + 1)) / 2) * 5000;
  ...
  update() {
    if (
      ...
      !this.controller.cutScenePlaying
    ) {
