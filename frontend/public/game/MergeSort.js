class MergeSort extends Phaser.Scene {
    ...
    create() {
        this.background=this.add.tileSprite(0,0,config.width,config.height,"forest");
        this.background.setOrigin(0,0);
        this.background.setScale(1,1.5);

        // this.createTrees();
        this.ground=this.add.rectangle(0,370,config.width, config.height-370,0x284e61);
        ...
        this.goLeft=new Button({ctx:this, x:config.width-250,y:300,btnName:"Choose Left",fontSize:15}).createButtons();
        this.goRight=new Button({ctx:this,x:config.width-250,y:this.goLeft.y+60, btnName:"Choose Right",fontSize:15}).createButtons()

        this.goLeft.visible=this.goRight.visible=false;

        this.divide=new Button({ctx:this,x:config.width-250,y:300,btnName:"Divide",}).createButtons();
        this.combine=new Button({ctx:this,x:config.width-250,y:this.divide.y+60, btnName:"Combine"}).createButtons()
    ...
    async mergeSort(begin, end, currentNode, currHeight) {
        if(!this.divideDone){
            ...
            await (new MessageBox({ctx:this})).startTyping(window.guidance.mergeSort.divide);
        }
        if (begin >= end) {
            return;
        }
        ...
        if(!this.mergeDone){
            this.mergeDone=true;
            await  (new MessageBox({ctx:this})).startTyping(window.guidance.mergeSort.merge);
        }
    ...
    async appendNumber(node,left, mid, right){
        return new Promise((resolve)=>{
            ...
            let nodeInd=0,leftInd=0, rightInd=0,mergeInd=left;
            ...
            this.goLeft.getByName("btn").on('pointerdown',()=>{
                if(leftInd<mid-left+1){
                    if(rightInd>=rightNode.length||parseInt(leftNode[leftInd].text)<=parseInt(rightNode[rightInd].text)){
                        ...
                        (new MessageBox({ctx:this})).startTyping(window.hints.mergeSort.misstep3)
            ...
            this.goRight.getByName("btn").on('pointerdown',()=>{
                if(rightInd<right-mid){
                    if(leftInd>=leftNode.length||parseInt(leftNode[leftInd].text)>=parseInt(rightNode[rightInd].text)){
    ...
    async merge(left, mid, right,currHeight, currNode) {
        const newY=config.height-(2*this.maxHeight-currHeight+1)*(this.nodeHeight+this.gapBetweenNodes)-this.startFrom;
    ...
    requestToDivide() {
        return new Promise((resolve) => {
            const combineClicked=()=>{
                (new MessageBox({ctx:this})).startTyping(window.hints.mergeSort.misstep1)
                this.rewind.reduceLive();
            }
    ...
    requestToMerge() {
        return new Promise((resolve) => {
            ...
            const divideClicked=()=>{
                (new MessageBox({ctx:this})).startTyping(window.hints.mergeSort.misstep2)
                this.rewind.reduceLive();
            }
